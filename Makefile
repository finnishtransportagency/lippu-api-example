...
keys/operator.key: keys/operator.pem
	openssl pkcs8 -topk8 -inform PEM -outform DER -in keys/operator.pem  -nocrypt > keys/operator.key
...
stamps/docker-dependencies: dirs
	sudo docker build --rm -f docker/Dockerfile-builddeps -t lippu/lippu-builddeps .
	touch $@
...
artifacts/lippu-service.jar: dirs stamps/docker-dependencies
	sudo docker build --rm -f docker/Dockerfile-build -t lippu/lippu-build .
