...
/**
 * Interface declaration for Reservation functionality. Methods to create and delete
 * reservation and
 * reservation codes.
 */
public interface IReservationService {
     ...
     */
    int delete(String caseId) throws NotAuthorizedException, ResourceNotFoundException;

    /**
     * Removes one reservation item. If its the only item, deletes also the reservation.
     *
     ...
     */
    ReservationItem activateReservationItem(String travelEntitlementId) throws
            NotAuthorizedException, ResourceNotFoundException;

    /**
     * Gets one reservation item from repository. Throws {@link NotAuthorizedException}
     * if client is not authorized to for the reservationItem.
     * Throws {@link NotFoundException} if the reservation item is not found.
     *
     * @param travelEntitlementId The id for travel entitlement.
     * @return Operation result code.
     */
    ReservationItem getReservationItem(String travelEntitlementId) throws
            NotAuthorizedException, ResourceNotFoundException;

    /**
     * Confirms previous reservation made with availability query. First it validates the
     * reservation requests and if all are valid, it confirms them.
     *
    ...
    List<ReservationItem> confirmReservation(List<ReservationRequestReservations>
                                                     reservations) throws
            NotAuthorizedException, NotValidReservationRequest, ResourceNotFoundException;
     ...
     * @return Count how many confirmed
     */
    ReservationItem createReservationItem(Product product, Reservation reservation,
                                          TravelRequest travel, TravelPassenger
                                                  passenger);
    ...
    /**
     * Generates reservation code for extra service to be included in the reservation
     * item.
     *
    ...
    /**
     * Generates reservation code for accessibility feature to be included in the
     * reservation item.
     *
     * @param service The service to generate the reservation code for.
