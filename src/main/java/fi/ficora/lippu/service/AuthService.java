...
public class AuthService implements IAuthService {
  ...
  @Autowired private NonceRepository nonceRepository;
  ...
  public static boolean checkSignature(String message, String sign, String keyPath, String alg) {
    ...
    try {
      Signature signature = Signature.getInstance(Constants.TOKEN_SIGNATURE_SHA256RSA);
  ...
  public Auth verifyAuthentication(
      String data, String cnonce, String snonce, String keyId, String alg)
    ...
    Client client = this.clientRepository.findOne(serverNonce.getClient());
    ClientKey key = this.findClientKey(client, keyId);
    if (key == null) {
      AuthService.log.info(
          "Did not find client's ({}) key with keyId {}", client.getAccount(), keyId);
      throw new AuthVerificationFailedException("Did not find client's key" + "with given keyId");
    }
    try {
      AuthService.log.debug("Decoding base64 payload {}", data);
      byte[] dataEncoded =
          Base64.getEncoder().encode((serverNonce.getNonce() + cnonce).getBytes("utf-8"));
      boolean isValidSignature =
          AuthService.checkSignature(
              new String(dataEncoded, Charset.forName("UTF-8")), data, key.getKeyfile(), alg);
      AuthService.log.debug("Payload is {} {}  {}", isValidSignature, dataEncoded, data);
      if (!isValidSignature) {
        AuthService.log.info("Payload signature was not valid");
        throw new AuthVerificationFailedException("Payload signature was not valid");
      }
      ...
      throw new AuthVerificationFailedException(
          "Exception while processing" + "authentication payload:" + e.getMessage());
    }
  }
  ...
  private Auth generateAuth(Client client) throws JoseException {
    OffsetDateTime exp = OffsetDateTime.now().plusMinutes(Constants.JWT_EXPIRATION_TIME_MINUTES);
   ...
   * @throws JoseException JWT Jose Exception
   */
  private String generateJWTWithJWS(Client client, OffsetDateTime exp) throws JoseException {
    try {
  ...
  private String generateJWTWithJWE(Client client, OffsetDateTime exp) throws JoseException {
    try {
      ...
      PublicKey publicKey = KeyUtil.getPublicKey(this.operatorConfiguration.getPublicKey());
  ...
  public Nonce generateNonce(String account) throws AccountNotFoundException {
    if (this.clientRepository.exists(account)) {
      ...
      LocalDateTime exp =
          LocalDateTime.now().plusSeconds(Constants.SERVER_SIDE_NONCE_TIMEOUT_SECONDS);
  ...
  public Nonce verifyNonce(String nonceValue, String client) {
    if (this.nonceRepository.exists(nonceValue)) {
      ...
      if (nonce.getExp().isAfter(LocalDateTime.now()) && nonce.getClient().equals(client))
        return nonce;
      else return null;
    } else {
  ...
  public void verifyAuthorization(ReservationItem item) throws NotAuthorizedException {
    ...
    if (!item.getClientId().equals(clientId)) {
      AuthService.log.warn(
          "Client is not authorized to access item. "
              + "Reservation item client id: {}, current client id {}",
          item.getClientId(),
          clientId);
      throw new NotAuthorizedException("Client not authorized" + "to access resource.");
    }
  }
  ...
  public void verifyAuthorization(Reservation reservation) throws NotAuthorizedException {
    ...
    if (!reservation.getClientId().equals(clientId)) {
      AuthService.log.warn(
          "Client is not authorized to access reservation. "
