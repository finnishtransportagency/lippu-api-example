...
public class ApiErrorUtil {
    ...
    public static ResponseEntity<ApiError> generateErrorResponse400(String message) {
        return new ResponseEntity<ApiError>(ApiErrorUtil.generateApiError400(message),
                HttpStatus.BAD_REQUEST);
    }

    public static ResponseEntity<ApiError> generateErrorResponse403(String message) {
        return new ResponseEntity<ApiError>(new ApiError().statusCode(BigDecimal
                .valueOf(403)).message(message), HttpStatus.FORBIDDEN);
    }

    public static ResponseEntity<ApiError> generateErrorResponse404(String message) {
        return new ResponseEntity<ApiError>(new ApiError().statusCode(BigDecimal
                .valueOf(404)).message(message), HttpStatus.NOT_FOUND);
    }

    public static ResponseEntity<ApiError> generateErrorResponse500(String message) {
        return new ResponseEntity<ApiError>(new ApiError().statusCode(BigDecimal
                .valueOf(500)).message(message), HttpStatus.INTERNAL_SERVER_ERROR);
    }
}
